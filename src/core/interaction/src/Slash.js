...
class Slash {
    ...
    async loadSlashCommands() {
        ...
        // Load from unified App directory
        ...
        try {
            ...
            try {
                const rows = db.get('command_settings', { guild_id: config.ServerID, is_active: 0 });
                if (rows && Array.isArray(rows)) {
                    rows.forEach(r => disabledCommands.add(r.command_name));
                }
            } catch (err) {
                console.error('Komut ayarlari Slash yuklenirken okunamadi:', err);
            }
            ...
            for (const file of commandFiles) {
                try {
                    ...
                    if ('data' in command && 'execute' in command) {
                        ...
                        if (!disabledCommands.has(commandData.name)) {
                            if (command.guildOnly) {
                                const guildId = config.ServerID || config.MainBotID;
            ...
            const appId = this.client.user?.id || config.MainBotID;

            if (globalCommands.length > 0) {
                await this.updateCommands(rest, Routes.applicationCommands(appId), globalCommands, 'Global'); 
            }

            for (const [guildId] of guilds) {
                if (guildCommands.has(guildId)) {
                    await this.updateGuildCommands(rest, appId, guildId, guildCommands.get(guildId) || []); 
                }
            }
        } catch (fsError) {
                ...
                for (const file of subcommandFiles) {
                    try {
                        ...
                        if ('execute' in subcommand) {
                            ...
                            const key = `${dirName.toLowerCase()}:${subcommandName}`;
    ...
    async handleInteraction(interaction) {
        ...
        console.log(`[Interaction] ${interaction.commandName} - ID: ${interaction.id}`);
        ...
        const ctx = new CommandContext(this.client, interaction, [], config);
        ...
        try {
            ...
            try {
                const setting = db.getOne('command_settings', { guild_id: interaction.guildId, command_name: interaction.commandName });
                if (setting && setting.is_active == 0) {
                    isDisabled = true;
                }
            } catch (dbErr) {
                console.error('Komut durumu kontrol edilemedi:', dbErr.message);
            }

            if (isDisabled) {
                return interaction.reply({
                    content: '❌ Bu komut sunucu yöneticileri tarafından devre dışı bırakılmıştır.',
                    ephemeral: true
                });
            }

...
class interactions {
    ...
    setupListeners() {
        const ContainerPatcher = require('../../utils/message/ContainerPatcher');

        this.client.on('interactionCreate', async (interaction) => {
            // ContainerBuilder'a otomatik çeviri yaması
            ContainerPatcher.patchInteraction(interaction);

            if (!interaction.guild || (allowedServerId && interaction.guild.id !== allowedServerId)) {
            ...
            try {
                ...
                if (interaction.isRepliable()) {
                    const errorMsg = { content: '❌ Bu işlem sırasında bir hata oluştu. Lütfen konsol kayıtlarını kontrol edin.', ephemeral: true };
        ...
        this.client.on('messageCreate', async (message) => {
            // ContainerBuilder'a otomatik çeviri yaması
            ContainerPatcher.patchMessage(message);

            if (!message.guild || message.author.bot) return;
            if (allowedServerId && message.guild.id !== allowedServerId) return;
